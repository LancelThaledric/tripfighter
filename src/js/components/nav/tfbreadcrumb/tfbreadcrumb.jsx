...
class TfBreadcrumb extends React.Component {
    ...
    render(){
        ...
        if(this.props.articleTitle && this.props.articleSlug)
            links.push(<Icon name="angle-right" key={'icon-article-'+this.props.articleSlug} className="tf-icon-last-breadcrumb"/>, <Link to={Url.computeUrl('', '', this.props.articleSlug)} key={'article-'+this.props.articleSlug} className="tf-icon-last-breadcrumb">{this.props.articleTitle}</Link>);
        ...
        if(switchLink.universSlug){
            switcherElement = <div className="tf-breadcrumb-switcher">
                ...
                <Icon name="plane" className="tf-switcher-plane"/>
            </div>;
        }
